...
function hasSupport() {
    ...
    hasSupport.support =
        typeof document.implementation != "undefined" && document.implementation.hasFeature("html", "1.0");
...
WebFXTabPane.prototype.addTabPage = function (oElement, callBackFnc) {
    // modifed by Raymond
    ...
    if (n == this.selectedIndex) tp.show();
    else tp.hide();
...
WebFXTabPane.getCookie = function (sName) {
    const re = new RegExp("(\;|^)[^;]*(" + sName + ")\=([^;]*)(;|$)");
    const res = re.exec(document.cookie);
...
WebFXTabPage.tabOver = function (tabpage) {
    const el = tabpage.tab;
    let s = el.className + " hover";
    s = s.replace(/ +/g, " ");
    el.className = s;
};
...
function setupAllTabs() {
    ...
    const l = all.length;
    const tabPaneRe = /tab\-pane/;
    const tabPageRe = /tab\-page/;
    let cn, el;
    let parentTabPane;
...
if (typeof window.addEventListener != "undefined") {
    ...
    window.addEventListener("unload", disposeAllTabs, false);
} else {
    ...
    } else window.onload = setupAllTabs;
    ...
    } else window.onunload = disposeAllTabs;
}
