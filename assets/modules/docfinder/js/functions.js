...
const startAjaxSearch = function () {
    ...
    if ($("replace_mode").value == 1) {
        ...
        if ($("regular_expression").getValue() == "on")
            infoText = "The replace function can not be combinded with regular expressions.";

        // abort if logical operators are used
        if (
            $("searchstring").value.test("AND") ||
            $("searchstring").value.test("OR") ||
            $("searchstring").value.test("NOT")
        )
            infoText = "The replace function can not be combined with logic operators.";

        // abort if the ALL operator is used
        if ($("searchstring").value == "ALL")
            infoText = "The replace function can not be combined with the ALL operator.";
        ...
        const confirmation = confirm(
            "Replace all instances of “" + $("searchstring").value + "” with “" + $("replacestring").value + "”?",
        );
