const vlaCalendar = new Class({
    ...
    initializeCalendarFunctions: function () {
        ...
        const vars = Json.evaluate(this.mainLoader.getElement("table").getProperty("summary"));
        const _class = this;

        //Change the label
        this.label.removeClass("noHover").setHTML(vars.label).onclick = vars.parent
            ? function () {
                  _class.u(vars.parent, "ts=" + vars.ts + "&parent=" + vars.current, function () {
                      _class.fade();
                  });
              }
            : null;
        ...
        this.arrowLeft.onclick = function () {
            _class.u(vars.current, "ts=" + vars.pr_ts, function () {
                _class.slideLeft();
            });
        };
        this.arrowRight.onclick = function () {
            _class.u(vars.current, "ts=" + vars.nx_ts, function () {
                _class.slideRight();
            });
        };

        //Add cell click events
        const clickables = this.mainLoader.getElements("td");
        switch (vars.current) {
            case "month":
                if (this.picker) {
                    clickables.each(function (_clickable) {
                        _clickable.onclick = function () {
                            _class.pick(Json.evaluate(_clickable.getProperty("date")));
                        };
                    });
                }
                break;
            ...
            case "decade":
                ...
                clickables.each(function (_clickable) {
                    _clickable.onclick = function () {
                        _class.u(
                            "year",
                            "ts=" + _clickable.getProperty("ts") + "&m_ts=" + _clickable.getProperty("m_ts"),
                            function () {
                                _class.fade();
                            },
                        );
        ...
        if (!this.loading) {
            ...
            new Ajax(this.filePath + _url + ".php", {
                ...
                data:
                    ...
                    (this.picker ? 1 : 0) +
                    "&startMonday=" +
                    (this.startMonday ? 1 : 0) +
                    "&leadingZero=" +
                    (this.leadingZero ? 1 : 0) +
                    "&twoDigitYear=" +
                    (this.twoDigitYear ? 1 : 0) +
                    "&style=" +
                    this.style,
                onComplete: function () {
                    _onComplete();
    ...
    slideLeft: function () {
        ...
        this.loading = true;
        this.tempLoader
            .effect("margin-left", { duration: this.slideDuration, transition: this.transition })
            .start(-164, 0);
        this.mainLoader
            .effect("margin-left", { duration: this.slideDuration, transition: this.transition })
            .start(0, 164)
    ...
    fade: function () {
        ...
        this.tempLoader
            .effect("opacity", { duration: this.fadeDuration, transition: this.transition })
            .start(0, 1)
            .addEvent("onComplete", function () {
    ...
    switchLoaders: function () {
        this.mainLoader =
            this.mainLoader.className == "loaderA"
                ? this.container.getElement("div[class=loaderB]")
                : this.container.getElement("div[class=loaderA]");
        this.tempLoader =
            this.tempLoader.className == "loaderA"
                ? this.container.getElement("div[class=loaderB]")
                : this.container.getElement("div[class=loaderA]");
...
const vlaDatePicker = vlaCalendar.extend({
    separateInput: false,
    separator: "/",
    format: "d/m/y",
    openWith: null,
    ...
    position: function () {
        ...
        switch (this.alignX) {
            case "left":
                ...
                break;
            case "center":
                let pickerMiddle = parseInt(this.pickerContent.getStyle("width")) / 2;
                if (pickerMiddle == 0) pickerMiddle = 83;
                left =
                    this.element.getLeft() +
                    this.element.getSize().size.x / 2 -
                    pickerMiddle -
                    (parseInt(this.pickerContent.getStyle("padding-left")) +
                        parseInt(this.pickerContent.getStyle("padding-right"))) /
                        2;
                break;
            case "right":
            default:
        ...
        switch (this.alignY) {
            ...
            case "top":
                top =
                    this.element.getTop() -
                    parseInt(this.pickerContent.getStyle("height")) -
                    (parseInt(this.pickerContent.getStyle("padding-top")) +
                        parseInt(this.pickerContent.getStyle("padding-bottom")));
                break;
            case "inputTop":
            default:
    ...
    show: function () {
        ...
        if (!this.visible) {
            ...
            this.picker
                ...
                .start(0, 1)
                .addEvent("onComplete", function () {
                    if (window.ie7) _class.picker.setStyle("background-color", "transparent");
    ...
    pick: function (_date) {
        if (this.separateInput) {
            ...
            switch (this.format) {
                case "m/d/y":
                    this.element.value = _date.month + this.separator + _date.day + this.separator + _date.year;
                    break;
                case "y/m/d":
                    this.element.value = _date.year + this.separator + _date.month + this.separator + _date.day;
                    break;
                case "y/d/m":
                    this.element.value = _date.year + this.separator + _date.day + this.separator + _date.month;
                    break;
                case "d/m/y":
                default:
                    this.element.value = _date.day + this.separator + _date.month + this.separator + _date.year;
            }
            this.hide();
        }
    },
    ...
    outsideClick: function (event, element) {
        ...
        const elementData = element.getCoordinates();

        return mousePos.x > elementData.left &&
            mousePos.x < elementData.left + elementData.width &&
            mousePos.y > elementData.top &&
            mousePos.y < elementData.top + elementData.height
            ? false
            : true;
    },

    getMousePos: function (e) {
        return { x: e ? e.pageX : 0, y: e ? e.pageY : 0 };
    },
});
